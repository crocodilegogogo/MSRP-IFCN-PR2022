...
def TRP_trans(x, m, tau, sign_flag, image_size):
    ...
    for mi in range(m):
        if mi == 0:
            ...
            xe = np.concatenate((xe, y[:, tau*mi: N2+tau*mi, :]), axis = 2) # 生成从y的1、5、9位置开始截取的长度为N2的向量  
    
    x1 = np.tile(xe, (1, N2, 1));                   #将xe的内容按行重复N2遍    
...
def TRP_images(input_data, per_interval_num, m, tau, sign_flag, image_size):
    
    TRP_images = []  
    
    if input_data.shape[0] <= per_interval_num:
        input_per_time = input_data[0:input_data.shape[0], :]
        TRP_images = TRP_trans(input_per_time, m, tau, sign_flag, image_size)    
    else:
        ...
        for i in range(interval):    
            if i != interval-1:
                ...
                if i == 0:
                    ...
                    TRP_images = np.concatenate((TRP_images,
                                                 TRP_trans(input_per_time, m, tau, sign_flag, image_size)),
                                                 axis=0)
            else:
                ...
                TRP_images     = np.concatenate((TRP_images, 
                                                 TRP_trans(input_per_time, m, tau, sign_flag, image_size)), 
                                                 axis=0)
    Max_TRP_images = np.max(np.max(TRP_images, axis = 1), axis = 1)
    T_Max_RP_images = Max_TRP_images*0.05
    T_RP_images = TRP_images-T_Max_RP_images.reshape(len(T_Max_RP_images),1,1) 
    # TRP_images = np.zeros(TRP_images.shape)
    T_RP_images[T_RP_images>=0] = 0
    T_RP_images[T_RP_images<0] = 1
    # TRP_images[T_RP_images<0] = 1
    return TRP_images
...
for dataset_param in UNIVARIATE_DATASET_NAMES:
    ...
    save_path = data_file_path+'//archives//TRP_UCR_Archive//'+dataset_id+'/'
    ...
    # TRP_images(input_data, interval_num, m, tau, sign_flag, image_size) # interval_num: the num of samples per interval
    MS_RP_imgs_train_store = TRP_images(train_x, 50, 3, 2, 0, image_size)
    MS_RP_imgs_test_store = TRP_images(test_x, 50, 3, 2, 0, image_size)
